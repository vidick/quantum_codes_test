...
Let  $(\mM,\tau^\mM)$ be a tracial von Neumann algebra, $P\in\mM_\infty$ a projection of finite trace, $\mN=P\mM_\infty P$ and $\tau^\mN=\tau_\infty/\tau_\infty(P)$, and $w\in P \mM_\infty \Id_\mM$ a partial isometry. \hnote{The $P$ here is unrelated to the projective measurement $\{P_a\}$, right? OK if $P$ is changed to, say, $\Pi$? (or alternatively $\{P_a\}$ is named something else.} Let 
...
Let $G = \langle S:R\rangle $ be a finitely presented group, $\mu$ a distribution on $R$, and $(\mM,\tau)$ a tracial von Neumann algebra. An $(\eps,\mu)$-almost homomorphism of $G$ on $(\mM,\tau)$ is a homomorphism $\phi:\mF(S)\to\mU(\mM)$ \hnote{I find it a little funny to call $\phi$ a ``homomorphism'' because the free group doesn't have any relations (maybe except for the identity)} such that
...
Let $G$ be a finite group and $\mC$ the class of all tracial von Neumann algebras. Let $U_S$ and $U_R$ be the uniform distribution on $S=G$ and $R=\{ g\cdot h \cdot (gh)^{-1}=e \}$ respectively. Then $G=\langle S:R\rangle$ is $(O(\eps),U_S,U_R,\mC)$-stable.  \hnote{$U_S,U_R$ may not be the best notation, as similar names are used for unitaries e.g., in the next theorem. Maybe just say $\mu_S,\mu_R$ are uniform?}
\end{theorem}
...
\[ H_{2k+1} = \left \{ \begin{pmatrix} 1 & a & c \\ 0 & 1_k & b \\ 0 & 0 & 1 \end{pmatrix} \right\}\subseteq {\mathrm{GL}}_{k+2}(\F_2)\;.\]
\hnote{what's the $1_k$ in the middle?}
 Concretely, we label elements of the Pauli group as triples $(J,a,b)$ \hnote{is this right? There should be an option to ``turn $J$ off'' right?} where $J$ is a distinguished symbol and $a,b\in\Z_2^k$, and consider the following presentation for it:
\begin{align*}
...
\end{align*}
\hnote{should the $J$ and $(a,b)$ be triples instead? (given the sentence about labelling Pauli group as triples...)}
...
\end{equation}
\hnote{should it be $1 \leq i \leq m$, $1 \leq j < j' \leq n$?}
...
\hnote{same comment about range of $i$,$j$}
\end{lemma}
...
\begin{proof}
This an immediate consequence of Theorem~\ref{thm:gh}. \hnote{Sorry, I'm missing something here. That theorem has to do with exhaustive presentations. Is $G(h_\had)$ an exhaustive presentation -- is this because checking $g(x) + g(y) = g(x+y)$ is checking all group relations? If so this could be pointed out explicitly.}
\end{proof}
...
The tester verifies this relation along a randomly chosen \emph{axis-aligned direction}. \hnote{added:} For all points $u \in \F_q^m$ and $j \in \{1,\ldots,m\}$, we say that the line through $u$ parallel to the $j$-th axis is the set of points $\{ u + ie_j : i \in \F_q \}$ where $e_j=(0,\ldots,0,1,0,\ldots,0)\in \F_q^m$, where the unique $1$ is in the $j$-th position. 
...
\begin{enumerate}
	...
	\item \hnote{already defined what $e_j$ is above, remove this?} Let $e_j=(0,\ldots,0,1,0,\ldots,0)\in \F_q^m$, where the unique $1$ is in the $j$-th position. 
...
\hnote{I'm a little confused by the description of this local tester... This seems to just make $r$ queries, right (but it says $rq$ above)? And it feels like to ``simulate'' the check from $\code$, one has to make $2q$ random queries to the Hadamard code to decode the symbol from $\F_q$, right?}
