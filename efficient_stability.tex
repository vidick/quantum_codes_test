...
\forall s\in S\ \colon \ \ \rho'(s)=\bigoplus_{\alpha\in \Phi} \rho(\alpha(s)).
    \]
    We denote by $\mM_\Phi=\bigoplus_{\alpha\in \Phi}\mM\subseteq \mM_\infty$ and by $\mM_\alpha$ the copy of $\mM$ at the $\alpha\in \Phi$ position.\tnote{Is it canonical what is the trace on $\mM_\Phi$? In the calculation below you assume a uniform weighted average, is it useful to say it?}\mnote{Adding a clarification:} Note that $\mM_\Phi$ embeds in $\mM_\infty$, e.g. in the coordinates $1,...,|\Phi|$, and inherits a trace from it by defining 
    \[
    ...
    \]
    for any function $f\colon \mF(S)\to \complex$. Therefore, \tnote{For the second line, is it clear that we get uniform over $R_i$?}
    \[
    \begin{split}
...
\end{corollary}
\begin{remark}
    By applying this construction on the example from Claim~\ref{claim:appendix1}, the resulting $\rho'$ is a $(\nicefrac{2|E|}{\binom{k}{2}},\infty)$-approximate representation of \eqref{eq:z2-efficient}.\tnote{Can we expand on this? Any permutation is an automorphism, right? So we end up having two classes of relations, and $w_1=w_2=1/2$. It might just be worth spelling out}\mnote{Done:} This is because $\Phi=\textrm{Sym}(S)$, and there are two  orbits --- the commutation relations and the involutions --- where $\mu_R$ is supprted equally on each of them. Namely, $w_{commutation}=w_{involution}=\nicefrac{1}{2}$.
\end{remark}
...
\]
Let $\varphi'\colon \Gamma\to \mU(\mN)$ be a genuine representation of $\Gamma$ which is $(\delta',\mu_S)$-close to $\rho'$, i.e.\ it satisfies \tnote{Is the $1/|\Phi|$ in front correct? Or maybe I'm missing how the $\tau_\infty$ is normalized exactly. It just sums over the $\alpha$'s?}\tnote{Also, I'm not sure I understood your starting point. You want to assume that $\varphi'$ is $L_\infty$-close, right? Here you only do $L_1$. Is this what the last sentence of the proof is supposed to justify? If so, perhaps better epxlain here.}\mnote{I added an explanation before the analysis, immediately after Remark A.5.}
\[
