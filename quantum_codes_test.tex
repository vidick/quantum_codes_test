...
When we write $\Es{i\in \mX}$ where $\mX$ is a finite set, we mean the expectation over $i$ chosen uniformly at random from $\mX$, i.e.\ $\frac{1}{|\mX|} \sum_{i\in \mX}$. \hnote{added:} For a vector $u \in \mX^n$ and a subset $S \subseteq [n]$, we write $u_S$ to denote the vector in $\mX^S$ which is the restriction of $u$ to $S$.
...
\hnote{remove this?} We end with a couple lemma that will prove useful. 
...
\item (Soundness:) For any $\eps\geq 0$ and any $u\in \F_q^n$ that is within (Hamming) distance at least  $\eps$ from $\code$, $\Pr_{S\sim\nu}(M_S(u_{|S})=0)\geq \delta(\eps)$.  \hnote{should it be $u_S$ or $u|_S$?}\tnote{What difference do you make between the two? We don't define either, so I'm fine using the one you find most intuitive/correct}\hnote{$u_S$ is fine by me (and we should say what this means in the preliminaries), I just saw both and thought we should be consistent.}
\end{itemize}
\end{definition}
