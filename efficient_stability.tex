...
To describe the tester we need to introduce interpolation coefficients, as follows. For $u,v\in \F_q$ and $i\in\{0,\ldots,d\}$ \hnote{here we're also canonically identifying some elements of $\F_q$ with $\{0,1,\ldots,d\}$, is this right? Or should it be a general collection of $d+1$ points $t_0,\ldots,t_d \in \F_q$?} let 
