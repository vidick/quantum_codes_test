...
For every integer $k\geq 1$ there is a presentation $\Z_2^k = \langle S_k:R_k\rangle$ such that $|S_k|,|R_k| = 2^{\poly\log(k)}$ and furthermore this presentation is $(\delta,\mu_{S_k},\mu_{R_k},d)$-stable for all $d \in \N$, where $\delta(\eps)=\poly(\log k,\eps)$, $\mu_{S_k}$ is uniform over the generators and $\mu_{R_k}$ is some distribution over the relations. \hnote{Spelled the conditions out because if we just let $\mu_S,\mu_R$ be anything, then we get a trivial statement if $\mu_S$, say, is just supported on one generator and $\mu_R$ is just on one relation.}
\end{theorem*}
...
\begin{lemma}\label{lem:lower_bound_on_stability_rate_standard_presentation_Z_2^k}
    ...
    There is a $(\nicefrac{1}{\binom{k}{2}},\mu_R)$-almost homomorphism of $\langle S\colon R\rangle$ which is at least $\nicefrac{1}{16k}$ far from any homomorphism. Namely, the modulus of stability of \eqref{eq:z2-efficient} is $\Omega(k\eps)$.\tnote{Is the `namely'' part immediate? Meaning, why could it not be $\Omega(\sqrt{\eps})$, which for large $\eps$ is better? Maybe you can get an example for a larger range of $\eps$ --- here $\eps$ depends on the size of the presentation. But maybe we can also just leave it at the first sentence.}\mnote{You are right. I meant that there is no better linear bound... Is there a succinct way of saying that? :)}
\end{lemma}
...
\end{align*}
but since $A_1$ permutes the basis of $\{e_v\otimes e_1\}$, we have
\begin{align*}
...
\begin{remark}
As was discussed in the introduction \mnote{Here we should refer to the discussion on the difference between $L^1$ and $L^\infty$ definitions},  it is more common to use the $L^\infty$ analogue of stability. A slight modification of the example given in  the proof of Lemma~\ref{lem:lower_bound_on_stability_rate_standard_presentation_Z_2^k} provides a lower bound also in that setup. The idea is as follows: In the construction we provided, the pair $(1,2)$ played a special role, since $A_1$ was a CNOT conditioned on the second bit. We can thus define the analogous construction for every pair $(i,j)$ where $1\leq i<j\leq k$. By tensoring all of these constructions along the pairs $(i,j)$, we get $k$ permutations acting on $\binom{[k]}{2}\times \field^{k+1}$, which in turn provide a lower bound in the $L^\infty$ setup. The proof is similar to the above, and since this is a slight digression from our main theme, we omit it from the text. 
\end{remark}

In the next section we will obtain presentations of $\Z_2^k$ that have a much better size/modulus tradeoff than the one given in~\eqref{eq:z2-efficient}. In the meantime we give one last example. To formulate it we recall the definition of the Pauli matrices
...
\begin{lemma}\label{lem:eff-pauli}
 ...
 the presentation~\eqref{eq:z2-efficient} is $(O(k\eps),\mu_S,\mu_R,\mC)$-stable.
\hnote{what's the dependence on $k$?}
\end{lemma}
