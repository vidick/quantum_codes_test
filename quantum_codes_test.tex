...
and let $\mu_{\dlS}$ be the uniform distribution over $S_X\times S_Z$. Let $\Omega$ be the support of $\mu_{\dlS}$ and $\alpha,\beta$ the coordinate projections. Then $G_{\dlS}=(\mX,\mu_{\dlS},\mA,D)$ has question set $\mX = \{PX,PZ\} \cup (\mX_{com}\times \Omega_+) \cup (\mX_{anticom} \times \Omega_-)$ and is as described in~\cite[Section 3.4]{de2022spectral}. 
...
\begin{figure}[!htbp]
  \centering
  \begin{gamespec}
Let $M$ be an $r$-local tester for the $[n,k,d]$ code $\code$. 
    \begin{enumerate}
...
Applying Lemma~\ref{lem:close-value} we deduce that the strategy that consists of $R^X$, $R^Z$, $P^{x_c,\omega}$ for $(x_c,\omega) \in \mX_{com}\times \Omega_+$ and $P^{x_{ac},\omega}$ for $(x_{ac},\omega)\in \mX_{anticom} \times \Omega_-$ succeeds in the game $G_{\dlS}$ with probability $1-O(\sqrt{\delta_1})$\tnote{should justify this with more detail?}. Applying~\cite[Corollary 3.9]{de2022spectral}, the conclusion follows. 
\end{proof}
