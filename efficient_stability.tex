...
\end{align}
Here a commutation relation between two generators  $[x_{j_1},x_{j_2}]=e$ is imposed only when needed for the relations $\prod_{1\leq j \leq n} x_j^{h_{ij}} = e$ to make sense, i.e.\ two generators are required to commute only if they both take part in the same equation, which is the case if and only if $h_{ij_1}h_{ij_2}=1$ for some $i$. \tnote{added:}While one could add all pairwise commutation relations, forcing $G(h)$ to be abelian --- and indeed we will do this in some cases later --- we choose the specific presentation~\eqref{eq:gh-intro} for consistency with the literature on non-local games, and particularly so-called \emph{linear constraint system games}, which we review in more detail below. 
...
\mnote{The following two paragraphs are problematic. The size of $G(h)$ is independent of $n,k$ in general, even if it is finite. The only case in which you can bound the size of $G(h)$ according to $n,k$ is when it is \textbf{abelian}, and we insisted on not assuming that. I feel like we should add all commutation relations before discussing this part.}\tnote{Yes so I tried doing this above, is it better? If we keep the notation, we need to modify around Lemma~\ref{lem:com-code} as well} 
...
Can such ``efficient'' presentations of $G(h)$ still be stable? The naive approach, of extending an $\eps$-approximate homomorphism of $G(h)=\langle S:R\rangle$ into an $\eps'$-approximate homomorphism of the multiplication table presentation of $\Z_2^k$, leads to $\eps'=\Omega(k^2\eps)$ and hence a logarithmic dependence of the modulus of stability on the group size. Is it possible to do better? Glebsky's result for the case of $\Z^2$ mentioned earlier suggests that it is in some cases  possible -- but in what generality? \mnote{Actually, we have a lower bound on the stability rate of $\Z^d$. It is $\Omega(\eps^{1/d})$ by a work of Becker and Mosheiff (they look at the permutation setup, but their example works for unitaries). Thus, there is not better solution in general, without assuming that all the generators are involutions. I just sent to John this draft, and by how he was confused by this paragraph, and the fact the the last question is known to be flase, we should rewrite it.}
...
\paragraph{Open questions.}\tnote{added this paragraph}
We leave many questions open. A natural direction is to determine if there are simple sufficient conditions on the matrix $h$ that guarantee that the presentation $G(h)$ is stable. As we discussed, the condition that $\ker h$ is a testable code is equivalent to stability for $1$-dimensional permutations. It thus seems likely that testability is not a sufficient condition in general; can testability be determined by a combinatorial parameter of $h$? More generally, finding other examples of efficient stability seems of intrinsic interest, besides potential applications to property testing and the construction of nonlocal games. In a different direction, an analogue of Theorem~\ref{thm:main-inf} for the case of representations in permutations could have important implications towards showing the existence of non-sofic groups, as is discussed in more depth in the works~\cite{CL_part2,BCLV_subgroup_tests}\tnote{Michael, can you add a reference to the relevant paper of yours/ours?}\mnote{Done. But, it seems as if the second paragraph of the 'related works on stability' subsection already hints towards this open problem.}
...
\label{sec:pres-code}\mnote{Why the same headline for the subsection as the section itself? Maybe we should change}\tnote{Sure, I changed}
...
Rubinfeld and Sudan~\cite{rubinfeld1996robust} (building on the work of Babai, Fortnow, and Lund~\cite{babai1991non}) showed that $h_{\RM}$ is indeed a parity check matrix for $\code_{\RM}$. Furthermore, they showed that the parity check matrix gives rise to a $(d+2)$-local tester for $\code_{\RM}$, whose soundness $\rho$ (as defined in the introduction) is at least $\frac{1}{6}$. \mnote{Clearly it is a tester, but shouldn't we remark the soundness parameter Rubinfeld--Sudan calculated?}\tnote{Sure, added}
...
A slight modification of the example given in  this appendix to prove Lemma~\ref{lem:lower_bound_on_stability_rate_standard_presentation_Z_2^k} provides a lower bound also in that setup. The idea is actually quite general, so we frame it in a somewhat general fashion. 
  ...
  Then, $\sigma$ extends (by the universal property of the free group) to an automorphism of $\mF(S)$ which we still denote by $\sigma$. 
...
\forall s\in S\ \colon \ \ \rho'(s)=\bigoplus_{\alpha\in \Phi} \rho(\alpha(s)).
    \]
    We denote by $\mM_\Phi=\bigoplus_{\alpha\in \Phi}\mM\subseteq \mM_\infty$ and by $\mM_\alpha$ the copy of $\mM$ at the $\alpha\in \Phi$ position.\tnote{Is it canonical what is the trace on $\mM_\Phi$? In the calculation below you assume a uniform weighted average, is it useful to say it?}
    Let $w_i=\mu_R(R_i)$, namely the probability that $\mu_R$ samples a relation from the orbit $R_i$. Then, for every $r\in R_i$, we have \tnote{For the second line, is it clear that we get uniform over $R_i$?}
    \[
    \begin{split}
        w_i\|\rho'(r)-\Id\|_\tau^2&=w_i\Expectation_{\alpha\in \Phi} \|\rho(\alpha(r))-\Id\|_\tau^2\\
        &=w_i\Expectation_{r'\in R_i} \|\rho(r')-\Id\|_\tau^2\\
        &\leq \sum_j w_j\Expectation_{r'\in R_j} \|\rho(r')-\Id\|_\tau^2\\
        &=\Expectation_{r\sim \mu_R}\|\rho(r)-\Id\|_\tau^2\leq \eps.
    \end{split}
    \]
    Hence, 
    \[
    \forall r\in R\ \colon\ \|\rho'(r)-\Id\|_\tau^2\leq \max({\nicefrac{1}{w_i}})\cdot\eps,
    \]
which in turn means that $\rho'$ is a $\max_i\{{\nicefrac{1}{w_i}}\}\cdot\eps$-approximate representation in the $L^\infty$ sense. 
\begin{remark}
    By applying this construction on the example from Claim~\ref{claim:appendix1}, the resulting $\rho'$ is a $\nicefrac{2|E|}{\binom{k}{2}}$-approximate representation of \eqref{eq:z2-efficient} in the $L^\infty$ sense.\tnote{Can we expand on this? Any permutation is an automorphism, right? So we end up having two classes of relations, and $w_1=w_2=1/2$. It might just be worth spelling out}
\end{remark}
...
\]
Let $\varphi'\colon \Gamma\to \mU(\mN)$ be a genuine representation of $\Gamma$ satisfying \tnote{Is the $1/|\Phi|$ in front correct? Or maybe I'm missing how the $\tau_\infty$ is normalized exactly. It just sums over the $\alpha$'s?}\tnote{Also, I'm not sure I understood your starting point. You want to assume that $\varphi'$ is $L_\infty$-close, right? Here you only do $L_1$. Is this what the last sentence of the proof is supposed to justify? If so, perhaps better epxlain here.}
\[
...
Since the $L^\infty$ distance between functions is larger than the $L^1$ distance between them (with respect to any fully supported measure $\mu_S$), we conclude that a lower bound of $f(\eps)$ on the modulus of stability of a sufficiently symmetric presentation induces a lower bound of $\frac{\min{w_i}}{3}f(\eps)$ \tnote{on? (I know, but maybe we should say?)}.
