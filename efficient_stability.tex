...
\begin{enumerate}
	\item \textbf{Low-degree test:}
		Let $u \sim \F_q^m$ be a uniformly random point and $j\sim \{1,\ldots,m\}$ chosen uniformly at random. Let $\ell$ be the line through $u$ in the $j$-th direction. Let $e_j=(0,\ldots,0,1,0,\ldots,0)\in \F_q^m$, where the unique $1$ is in the $j$-th position. Choose a uniformly random $v\in \ell$ and $\gamma\in \F_q$ and check that 
...
\begin{proof}
Due to the test of the relations $\{R^\com_k\}$, it holds that \tnote{this will depend on the distribution placed on commutation relations; the factor $4$ might be off} 
\begin{align}
