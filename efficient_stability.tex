...
\tnote{more? open questions?}\mnote{If we have open problems, maybe we should collect them at the end? Not sure about that. Did you think of any open problems other than the permutation analogue of the quantum soundness of the low-deg test?}\tnote{There's the obvious open problem of finding an LTC that is not stable (since I'd conjecture it this way), and conversely other examples of codes that are stable. We can also ask how far things can be pushed; in principle we could have stability, in our sense, for a finite pres of an infinite group.}\mnote{Agree about most of the problems you suggested. Regarding the infinite case, there are many stability results for infinite groups... We should be precise in what we mean by efficient in the infinite case. Maybe there is a way of formulating it. Actually, I do not know of any group which is infinite, not free or a product of a free and a finite group, which has linear stability rate...}
...
Rubinfeld and Sudan~\cite{rubinfeld1996robust} (building on the work of Babai, Fortnow, and Lund~\cite{babai1991non}) showed that $h_{\RM}$ is indeed a parity check matrix for $\code_{\RM}$. Furthermore, they showed that the parity check matrix gives rise to a $(d+2)$-local tester for $\code_{\RM}$. \mnote{Clearly it is a tester, but shouldn't we remark the soundness parameter Rubinfeld--Sudan calculated?}
