...
\hnote{added the parity check version of this test:} A parity check matrix $h_{\RM} \in \F^{m q^{m+1} \times q^m}$ for $\code_{\RM}$ is as follows. Identify the rows with triples $(u,j,t) \in \F_q^m \times \{1,\ldots,m\} \times (\F_q \setminus \{t_0,\ldots,t_d\})$. The $(u,j,t)$-th row of $h_{\RM}$ is the vector in $\F_q^m$ that has $\alpha_{u,u+te_j,i}$ in the coordinate indexed by $u + t_i e_j$, and $-1$ in the coordinate indexed by $u + te_j$. Rubinfeld and Sudan \hnote{need to double check reference} showed that $h_{\RM}$ is indeed a parity check matrix for $\code_{\RM}$, and... \hnote{complete later}
