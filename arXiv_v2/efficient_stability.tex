...
\tnote{removing erroneous dp fact}
...
\end{equation}
For any $i\in\{1,\ldots,n\}$ let $Q^{W,i}_b = \sum_{u:\,u_i=b}  Q^W_u$. \tnote{Added comment here to replace erroneous DP fact:}Expanding both terms on the left-hand side and using that $\tau(Q^W_u \tilde{P}^W_{u'})\geq 0$ for all $u,u'$,
\begin{align*}
