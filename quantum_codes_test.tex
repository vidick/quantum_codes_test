...
Let $\code$ be an $[n,k,d]_q$ linear code and $M$ be an $r$-local tester for $\code$. An \emph{$\eps$-local presentation of $(\code,M)$} is a pair $((\mM,\tau),A)$ of a tracial von Neumann algebra $(\mM,\tau)$ and a collection of projective measurements $A = \{A^i_a\}_{a\in\F_q} \subseteq\mM$ for $i\in\{1,\ldots,n\}$ such that for any $S\subseteq\{1,\ldots,n\}$ of size $|S|\leq r$ there are $|S|$ pairwise commuting projective measurements $\{B^{S,i}\}_{i \in S }\subset \mM_\infty$, with $B^{S,i} = \{B^{S,i}_b\}_{b\in \F_q}$, such that 
\begin{itemize}
