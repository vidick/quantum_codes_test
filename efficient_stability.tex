...
{\color{red} YOU ALREADY SAY THAT YOU ARE GIVEN A HOMOMOTPHISM FROM THE FREE GROUP. SO $\phi(r)$ IS WELL DEFINED... NO NEED FOR THE REMARK "Here ...." AT THE END}\tnote{Ok I erased it. Michael, can you mark your comments using the command ``$\backslash$mnote$\{\cdots\}$'', {\huge AND NOT USE CAPS PLEASE}}.
\end{definition}
...
\begin{remark}
\hnote{promoted this from footnote to remark}\tnote{Can we cancel this change? It's a remark below, see Remark~\ref{rk-mus}. But in the intro, it doesn't seem so crucial to highlight} It is natural, and will always be the case for us, to take $\mu_S$ as the ``marginal'' of $\mu_R$, i.e.\ the distribution on $S$ obtained by sampling $r\sim\mu_R$ and returning a uniformly random $s$ that appears in $r$, when $r$ is considered as a multi-set.
\end{remark}
