...
\hnote{edited slightly} An \emph{$r$-local $\delta$-tester for $\code$} is a pair $(M,\nu)$ where $\nu$ is a distribution over subsets $S\subseteq [n]$ of size at most $r$, and $M$ is a collection of predicates $M(S,\cdot):\F_q^S\to\{0,1\}$ for each subset $S$ such that the following hold:
\begin{itemize} 
\item (Completeness:) For any $u\in \code$, $\Pr_{S\sim \nu}( M(S,u_S)=1)=1$.
\item (Soundness:) For any $\eps\geq 0$ and any $u\in \F_q^n$ that is within (Hamming) distance at least  $\eps$ from $\code$, $\Pr_{S\sim\nu}(M(S,u_{|S})=0)\geq \delta(\eps)$.  \hnote{should it be $u_S$ or $u|_S$?}
\end{itemize}
\end{definition}
...
Let $\code$ be an $[n,k,d]_q$ linear code and $(M,\nu)$ \hnote{was: $M$ but it also depends on distribution $\nu$} be an $r$-local tester for $\code$. An \emph{$\eps$-local presentation of $(\code,M)$} \hnote{should this be $(\code,M,\nu)$?} is a pair $(\mM,A)$ of a tracial von Neumann algebra $\mM$ and a collection of projective measurements $A = \{A^i_a\}_{a\in\F_q} \subseteq\mM$ for $i\in\{1,\ldots,n\}$ such that for any $S\subseteq\{1,\ldots,n\}$ of size $|S|\leq r$ there are $|S|$ pairwise commuting projective measurements $\{B^{S,i}_b\}_{b\in \F_q}$, for $i\in S$, such that 
\begin{itemize}
...
Let $\code$ be an $[n,k,d]_q$ linear code and $(M,\nu)$ a $r$-local tester for $\code$. Let $\delta:[0,1]\to[0,1]$ be such that $\delta(0)=0$. We say that $(M,\nu)$ has \emph{quantum soundness $\delta(\eps)$} if the following holds. For any $\eps$-local presentation $(\mM,A)$ of $(\code,M)$
% such that 
