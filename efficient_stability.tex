...
\begin{definition}[Efficient stability]\label{def:eff-stab-intro}
...
We do not know of any other presentation of $\Z_2^k$, which is arguably the simplest example one could think of, that is stable with similar parameters as the one stated in the theorem. It would be very interesting to discover different such presentations, built from locally testable error-correcting codes or not, and for this group or other groups. 

\paragraph{Nonlocal games in quantum information theory.}

To motivate the focus on efficient stability we now sketch a connection between our results and problems in quantum complexity theory and in particular the theory of nonlocal games that motivate us. A \emph{nonlocal game} $\game$ is specified by the following date: finite question and answer sets $\mX$ and $\mA$ respectively, a distribution $\mu$ on $\mX\times \mX$, and a decision predicate $D:\mX\times \mX\times \mA\times \mA\to \{0,1\}$. The interpretation of $G=(\mX,\mu,\mA,D)$ as a game is as follows. A ``referee'' is imagined to sample a pair of ``questions'' $(x,y)\sim \mu$. Each question is sent to a different player, who is tasked with responding with an answer $a,b\in \mA$ respectively. Finally, the referee decides that the players win the game if and only if $D(x,y,a,b)=1$. Interestingly, the maximum success probability of the players in this game, probability is over the referee's choice of questions and any randomness in the player's strategy, depends on the set of strategies that is allowed. In particular, and without going into details, this maximum can be different depending on whether only \emph{classical} strategies are allowed (where each player answers according to a function $f:\mX\to\mA$ of their choice), or \emph{quantum} strategies are allowed, which allow some additional form of coordination between the players in the form of \emph{quantum entanglement}. Understanding when there is a gap between the two values, and how large the gap can be, is of great interest in the foundations of quantum mechanics. To study this question one is drawn to investigate the structure of optimal quantum strategies in a game, and how to design games that enforce a specific structure --- for example, forcing as much ``non-classicality'' in the strategy as possible. (And beyond that, many applications to e.g.\ verification which we do not survey in detail.)

A class of games that has been successful as LCS. These are defined by a system of equations. We now see the connection with presentations of the form $G(h)$. The stability results enable conclusions about approximately optimal strategies. In particular, the results about the Pauli group lead to qubit and dimension tests. These are an ingredient in MIP*=RE. Recover PBT


\paragraph{Related works on stability.}
...
Suppose given a finite presentation of a (possibly infinite) group $G$ using generators $S$ and relations $R$. Informally, we say that the presentation is \emph{stable} if any map from $S$ to e.g.\ unitaries that approximately respects the relations $R$ is close to a representation of $G$. Furthermore, we will say that the presentation is \emph{efficient} if it is stable and provides a good trade-off between its size (the number of relations used and their length) and how the closeness to a representation depends on the error in satisfying the relations $R$. All the notions referred to informally in the preceding sentences --- ``approximately,'' ``close,'' good trade-off,'' etc., can be formalized in a variety of ways, leading to often incomparable notions. Here we present the formalization that is most natural to us, and is the one suggested by applications to quantum information and complexity. 
...
We start with the notion of an almost-homomorphism, which formalizes what it means for a map defined on $S$ to approximately satisfy the relations $R$. The notion we give is a small variant on the notion of $\eps$-\emph{almost homomorphism} from a finitely presented group to a unital tracial $C^*$-algebra $\mA$ introduced in~\cite[Section 2]{hadwin2018stability}. %Informally, an $\eps$-almost homomorphism of $G=\langle S:R\rangle$ is a map from $S$ to $\mU(\mA)$ that approximately respects all relations in $R$. 
We give a variant of their definition that quantifies the error in an average sense. Below, when $\mu$ is a distribution over a finite set $\mX$ and $f:\mX\to \mR$ we write $\Es{r\sim \mu} f(r)$ for the expectation of $f$ under $\mu$. 
...
\begin{definition}[Efficient stability]\label{def:eff-stab}
...
Many results about various notions of stability are known. \tnote{insert more discussion here, or in the introduction} We give two results that are most relevant to our focus. First of all, 
...
The multiplication table presentation of $\Z_2^k$ is not very ``efficient'', in the sense that it has as many generators as the group size, and quadratically as many relations. There is a much more ``efficient'' presentation of $\Z_2^k$, given by

\begin{equation}\label{eq:z2-efficient}
 \Z_2^k = \langle x_1,\ldots,x_k : [x_i,x_j]=e, x_i^2=e \; \forall i\neq j \rangle\;.
\end{equation}

We call this presentation ``efficient'' because it has a number of generators and relations that is far smaller than those of the multiplication table presentation (in this case, polylogarithmic in the group size). The following lemma shows that this presentation is also stable. 
