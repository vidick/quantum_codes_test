...
Let $\{P_i\}$ and $\{Q_i\}$ be two projective measurements such that $P$ and $Q$ are $(\eps,\mu)$-close. Then 
\begin{equation}\label{eq:l1-l2}
\sum_i \tau\big(|P_i-Q_i|\big) \,\leq\, \eps+ 2 \sqrt{\eps}\;.
\end{equation}
\end{lemma}

\begin{proof}
Write, using the triangle inequality, 
\begin{align*}
 \sum_i \tau\big(|P_i - w^* Q_i w|\big) &\leq\, \sum_i \tau\big(|(P_i-P_i^2)|\big) + \tau\big(|P_i(P_i-w^*Q_iw)|\big) + \tau\big(|(P_i-w^*Q_iw)w^*Q_iw|\big) \\
&\qquad +\tau\big(| (w^*Q_iw^*wQ_iw-w^*Q_iw)|\big)\;.
\end{align*}
The first term on the right-hand side is zero, because $P$ is assumed projective. The terms in the middle are bounded using H\"older's inequality. The last term can be written as 
\begin{align*}
...
\end{align*}
The second and third term on the right-hand side can be bonded as
\begin{align*}
...
&\leq \eps\;,
\end{align*}
where the first inequality is by H\"older's inequality and the last inequality uses the definition of closeness and $\|A\circ B\| \leq \max_{i,j} |A_{i,j}|\|B\|$, with $A = \sum_{a,b} D(a,b) P_{a,b}$, with $P_{a,b} = \sum_{i,j} \bra{u_{i,a}} \ket{v_{j,b}}$ with $\ket{u_{i,a}}$ a basis for the range of $Q^x_a$ and $\ket{v_{j,b}}$ for the range of $Q^y_b$, and $B=\sum_{a,b} Q^y_b ww^* Q^x_a = ww^*$. 

For $x\in \mX$ such that $\mu(x)\neq 0$, denote by $\mu_x$ the (symmetrized) conditional distribution $\mu_x(y)=\frac{1}{2}(\mu(x,y)+\mu(y,x))/\mu(x)$. Then
\begin{align*}
\big|\omega(G;\strategy) - \omega(G;\strategy')\big|
 &\leq \eps+ \Big|\Es{(x,y)\sim\mu} \sum_{a,b} D(a,b,x,y) \big( \tau\big( P^x_a \, P^y_b \big)-\tau\big( w^* Q^x_a w\, w^* Q^y_b w\big)\big)\Big|\\
&\leq  \eps+ \Big|\Es{x\sim\mu} \sum_{a}  \tau\Big( \big(P^x_a-w^* Q^x_aw\big) \, \Big( \Es{y\sim \mu_x} \sum_b D(a,b,x,y) P^y_b \Big)\Big)\Big|\\
&\qquad+ \Big|\Es{y\sim\mu} \sum_{b}  \tau\Big(\Big( \Es{x\sim \mu_y} \sum_a D(a,b,x,y) w^* Q^x_a w\Big) \big(P^y_b-w^*Q^y_bw\big) \, \Big)\Big|\\
&\leq \eps+2\Es{x\sim\mu} \sum_{a}  \tau\big(\big| P^x_a-w^*Q^x_aw\big|\big) \\
&\leq \eps+4\sqrt{\eps}\;,
\end{align*}
...
\begin{enumerate}
\item If $M$ has quantum soundness $\delta$ then $G_{\code,M}$ is $\delta''$-robust, for some $\delta''(\eps)=O(r\sqrt{\delta(2\eps)})$. 
\item If $G_{\code,M}$ is $\delta'$-robust and $(\mC,M)$ is Abelian then $M$ has quantum soundness $\delta''$, for some $\delta''(\eps) = O(\delta'(\sqrt{\eps}))$. 
\end{enumerate}
\end{proposition}

\begin{proof}[Proof sketch]
We start with the second assertion. Suppose then that $G_{\code,M}$ is $\delta'$-robust, for some function $\delta'$. Let $(\mM,A)$ be be an $r$-local $\eps$-presentation of $\code$, and for each $S\subseteq\{1,\ldots,n\}$ such that $|S|=r$, $\{B^{S,i}\}_{i\in S}$ the commuting family of projective measurements promised by the definition. Then for every $S$ then there is a projection $P^S$ in $\mM_\infty$ and an isometry $w^S\in P^S \mM_\infty 1_\mM$ such that 
...
\end{align}
Let $\{P^S_a\}$ be the family of projective measurements on $\mM$ that is promised by Lemma~\ref{lem:pull-back}, so that by a simple averaging argument,
...
\begin{equation}
 \sum_{(S,i)} \mu(S,i) \sum_{a\in \field^s}\sum_{b\in\field} D(S,i,a,b) \tau\big(  P^S_a  A^i_{b}\big) \,\geq \,1-\eps\;.\label{eq:s-g-3}
\end{equation}
For each $S$ and $i\in S$, let $B^{S,i}_b = \sum_{a:a_i=b} P^S_a$. Then using the definition of $\mu$ and $D$, rewriting~\eqref{eq:s-g-3} immediately gives
\begin{equation}
...
For every $S$, let $Q^S_a = \prod_{i\in S} C^i_{a_i}$, which is a projective measurement on $\mN$. It is easy to verify that $(\mN,C,Q)$ is a perfect strategy in $G_{\code,M}$. Moreover,
\begin{align}
...
\end{align}
Applying a simple inductive argument, using Lemma~\ref{lem:l1-l2} we get that
\begin{align*}
...
\begin{theorem}
Let $\code$ be an $[n,k,d]$ linear code and $M$ an $r$-local tester for $\code$ that is $\delta(\eps)$-quantum sound. Then the braiding test over $\code$ is a $(k,\delta')$-qubit test, where $S_X=S_Z=\{G_\code e_i:\,i\in\{1,\ldots,n\}\}\subseteq \field^k$, $\phi(W,G_\code e_i)=(W,i)$ and $\delta' = O(\sqrt{r}\delta^{1/4}(12\eps))$.\footnote{Assume $G_\code$ has no repeated columns.} 
\end{theorem}

\begin{proof}
\underline{Completeness:} We first verify completeness. For $W\in\{X,Z\}$ and $i\in\{1,\ldots,n\}$ let $P^{(W,i)}_a = (\tau^W(G_\code e_i))_a$ and $P^{(W,S)}_a = \prod_{i\in S} P^{(W,i)}_{a_i}$. For $(i_X,i_Z)\in \{1,\ldots,n\}^2 $ let $\omega=(G_\code e_{i_X}, G_\code e_{i_Z})$ and $\gamma =(G_\code i_X) \cdot(G_\code {i_Z}) $ we let $P^{x_{W,\gamma},\omega} = P^{(W,i_W)}$. 
...
\underline{Soundness:} Next we show soundness. Let $\strategy$ be a synchronous strategy for $G$  in  $(\mM,\tau)$ that succeeds with probability at least $1-\eps$. For $W\in\{X,Z\}$ let $\strategy^W$ be the strategy in $G_{\code,M}$ that is obtained by restricting $\strategy$ to the relevant measurements, i.e.\ the $P^{W,S}$ and $P^{W,i}$.  Then $\strategy^W$ succeeds with probability at least $1-6\eps$ in $G_{\code,M}$. Using the assumption that $M$ is $\delta(\cdot)$-quantum sound and the first item from Proposition~\ref{prop:sound-game} it follows that there is a $\delta_1 = O(r\sqrt{\delta(12\eps)})$ such that for each $W\in\{X,Z\}$, $\strategy^W$ is $(\delta_1,\mu)$ close to a perfect strategy $\tilde{\strategy}^W$ on $(\mN^W,\tau^W)$. This strategy in particular has measurement operators $\{ \tilde{P}^{W,S}_a\}$ and $\{\tilde{P}^{W,i}_b\}$ associated with questions of the form $S$ and $i$ in $G_{\code,M}$. Since $(\code,M)$ is Abelian, there is a  POVM $\{\tilde{P}^W_u\}_{u\in \field^n}$ such that $\sum_{u\in \code}\tilde{P}^W_u = \Id$ and for each $i\in \{1,\ldots,n\}$, $\tilde{P}^{W,i}_b = \sum_{a:a\cdot u_i=b} \tilde{P}^W_a$ where  $u_i = G_\code e_i$.
...
\end{equation*}
For any $i\in\{1,\ldots,n\}$ let $Q^{W,i}_b = \sum_{a:\,a\cdot u_i=b}  Q^W_a$. Then by Lemma~\ref{lem:dp},
\begin{align*}
\Es{i}\sum_b \tau\big( Q^{W,i}_b (w^W)^*\tilde{P}^{W,i}_b(w^W) \big)
&\geq \Es{i}\sum_a \tau\big( Q^{W}_a (w^W)^*\tilde{P}^{W}_a(w^W) \big)\\
&\geq 1-O(\delta_1)\;.
\end{align*}
Applying Lemma~\ref{lem:close-value} we deduce that the strategy that consists of $Q^X$, $Q^Z$, $P^{x_c,\omega}$ for $(x_c,\omega) \in \mX_{com}\times \Omega_+$ and $P^{x_{ac},\omega}$ for $(x_{ac},\omega)\in \mX_{anticom} \times \Omega_-$ succeeds in the game $G_{\dlS}$ with probability $1-O(\sqrt{\delta_1})$. Applying~\cite[Corollary 3.9]{de2022spectral}, the conclusion follows. 
\end{proof}
