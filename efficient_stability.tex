...
\hnote{maybe good to add a sentence or two of intuition about why this tester makes sense...}\tnote{added}

\hnote{Also, probably could use another lemma or proposition here arguing that this is in fact a valid tester.}\tnote{added above, if this what you meant?}
...
To show this it suffices to verify that we only need to ``commute'' pairs of terms whose exponents are independent and uniformly random. Using Claim~\ref{claim:z2-stab-3} twice, the right-hand side satisfies \hnote{the first parenthesis should have $(-1)^{\tr(\sum y_i \alpha_i)}$, right?}
...
\begin{equation}\label{eq:z2-stab-4-1}
 ...
 \Es{\gamma\in\F_q} \Big\| U_v^\gamma -  \sum_{g} (-1)^{\tr(\gamma \sum \alpha_{u_\ell,v,i} g(u_\ell+t_ie_j))} Q^{\ell}_g\Big\|_\tau^2 \,=\, \poly(d,\eps)\;.
	\end{equation}
	\hnote{What does $U_v^\gamma$ mean?} By definition, $\sum \alpha_{u_\ell,v,i} g(u_\ell+t_ie_j) = g(v)$. By Fourier transform, we obtain the desired conclusion. 
\end{proof}
...
where $\eta = \poly(m,d,r) \cdot\poly(\eps,q^{-1},e^{-\Omega(r/m^2)})$, with $r\geq 12mt$ an (arbitrary) integer. \hnote{what is the quantification on $r$? Is it for all $r \geq 12mt$, or for some $r \geq 12mt$?}
