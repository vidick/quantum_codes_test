...
\end{align*}
\hnote{There seems to be some missing connective text between these two eqn blocks. Is it the commented out stuff below?}\tnote{fixed}
%Here, the second line follows since for $\beta,\beta'\in \F_q$, 
%\[ \Es{\alpha\in \F_q} (-1)^{\alpha(\beta-\beta')}\,=\delta_{\beta,\beta'}\,=\,\Es{\alpha\in\F_2^t} (-1)^{\alpha\cdot \kappa(\beta-\beta')}\,\;.\]
...
\begin{equation}\label{eq:z2-stab-4-1}
 ...
 \Es{\gamma\in\F_q} \Big\| U_{v,\gamma} -  \sum_{g} (-1)^{\tr(\gamma \sum \alpha_{u_\ell,v,i} g(u_\ell+t_ie_j))} Q^{\ell}_g\Big\|_\tau^2 \,=\, \poly(d,\eps)\;.
	\end{equation}
	\hnote{What does $U_v^\gamma$ mean?}\tnote{old notation; fixed} By definition, $\sum \alpha_{u_\ell,v,i} g(u_\ell+t_ie_j) = g(v)$. By Fourier transform, we obtain the desired conclusion. 
\end{proof}
...
where $\eta = \poly(m,d,r) \cdot\poly(\eps,q^{-1},e^{-\Omega(r/m^2)})$, with $r\geq 12mt$ an (arbitrary) integer. \hnote{what is the quantification on $r$? Is it for all $r \geq 12mt$, or for some $r \geq 12mt$?}\tnote{That's from our paper :) You get to choose $r$. That's what I meant by ``arbitrary''. Feel free to edit} 

\end{proof}


\section{Application to testing entanglement}

In this section we give an application of our stability results to the problem of entanglement testing. We first introduce the language of nonlocal games. Then we associate a nonlocal game to any presentation of $\Z_2^t$. Finally, we show that, if the presentation is stable, then the game is a good entanglement test. 
...
We introduce a game, which we denote $G_{\dlS}$, that is a specific instantiation of a more general class of games analyzed in~\cite[Section 3.4]{de2022spectral}.  Using notation from~\cite{de2022spectral}, the game $G_{\dlS}$ is obtained by making the following choices. The group is $H=\field^k$. Let
...
where $|h_{j\cdot}|$ denotes the Hamming weight of the $j$-th row of $h$. For any $j$, $\mA((\eq,j))=\F_2^{|h_{j\cdot}|}$, and for any $i\in\{1,\ldots,n\}$, $\mA((\var,i))=\F_2$. Finally $D((\eq,j),(\var,i),a,b)=(h_{j\cdot} a) 1_{a_i=b}$, where $h_{j\cdot}a$ is naturally computed as the sum, in $\F_2$, of all entries of $a$.  
\end{definition}

We show the following. 

\begin{proposition}\label{prop:rep-game}
Let $\code$ be an $[n,k,d]_2$ linear code and $M=(h,\nu)$ an $r$-local tester for $\code$. Let $\strategy$ be a strategy for $G_{\code,m}$ on $(\mN,\tau^\mN)$ such that $\omega^*(\G_{\code,M},\strategy)\geq 1-\eps$. Then $\{\phi((\var,i))\}_{1\leq i \leq n}$ is an $\eps$-homomorphism of the presentation~\eqref{eq:def-gh-pres} of $G(h)$. 
\end{proposition}
