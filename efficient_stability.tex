...
\begin{definition}
    Let $G=\langle S\colon R\rangle$ be a finite presentation. The \emph{length} of the presentation will be $\log|S|\cdot \sum_{r\in R}|r|,$ where $|r|$ is the length of the word $r\in \mF(S)$ written in the basis $S$.
\end{definition}
This is because we prefer to keep the notion of efficiency qualitative; furthermore, we do suggest a natural parameter setting which we will call ``efficient'', and which is good for certain applications.
